export function useExportPDF() {
  const exportToPDF = (title: string, dateRange: string, content: string) => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>${title} Mintex Staffing Report</title>
        <style>
      ...
      <body>
        <h1>${title}</h1>
        <div class="subtitle">Mintex Staffing Report • ${dateRange} • Generated ${new Date().toLocaleDateString()}</div>
        ${content}
        <div class="footer">Generated by Mintex Staffing KPI Dashboard</div>
      </body>
      </html>
    `);
